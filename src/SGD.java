...
//		B=DataSet.readDataSet("testSet.txt");
		...
		for (int iter = 1; iter <= numIterations*(numUsers*numFactors+numFactors*numItems); iter++) {
			Random rand=new Random();
			Random rand1=new Random();
			Random rand2=new Random();
			if(rand.nextInt(numUsers*numFactors+numFactors*numItems)<numUsers*numFactors){
				int userIdx=rand1.nextInt(numUsers-1);
				int factorIdx=rand2.nextInt(numFactors-1);
				...
				int factorIdx=rand1.nextInt(numFactors-1);
			...
			if(iter%(numUsers*numFactors+numFactors*numItems)==0){
